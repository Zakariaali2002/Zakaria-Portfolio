    ...
    <footer className="relative overflow-hidden border-t border-line">
        ...
        >
          <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-2 text-fg/60">Free consultation — response within 24 hours.</p>
            </div>
            <a
              href="#contact"
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-14">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1.3fr]">
            ...
            <div className="mt-6 flex gap-3">
              {socials.map((s) => (
                <motion.a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-line bg-white/5 text-fg/60 transition hover:border-brand/60 hover:bg-brand/20 hover:text-fg hover:shadow-lg hover:shadow-brand/20"
                >
          ...
          {/* Quick links */}
          <div>
            <h4 className="font-display mb-5 font-semibold text-fg">Quick Links</h4>
            <ul className="space-y-3">
              {quickLinks.map((l) => (
                <li key={l.label}>
                  <a
                    href={l.href}
                    className="group inline-flex items-center gap-2 text-sm text-fg/55 transition hover:text-fg"
                  >
          ...
          {/* Services */}
          <div>
            <h4 className="font-display mb-5 font-semibold text-fg">Services</h4>
            <ul className="space-y-3">
              {services.map((s) => (
                <li key={s.label}>
                  <a
          ...
          {/* Contact + newsletter */}
          <div>
            <h4 className="font-display mb-5 font-semibold text-fg">Get In Touch</h4>
            <ul className="space-y-3.5">
              {contactInfo.map((c) => (
                <li key={c.value}>
                  <a href={c.href} className="group flex items-center gap-3 text-sm text-fg/55 transition hover:text-fg">
            ...
            <form onSubmit={handleSubscribe} className="mt-6">
              <p className="mb-2.5 text-sm font-medium text-fg/70">Subscribe to my newsletter</p>
              <div className="flex overflow-hidden rounded-xl border border-line bg-white/5 focus-within:border-brand/60">
                <input
                  type="email"
                  required
                  placeholder="Your email"
                  className="w-full bg-transparent px-4 py-3 text-sm text-fg placeholder-fg/30 outline-none"
                />
                <button
                  type="submit"
        ...
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-5 py-6 sm:flex-row">
          ...
          >
            ...
            <span className="hidden items-center gap-1.5 sm:flex">
              <span className="ml-2 h-1 w-1 rounded-full bg-fg/30" />
          ...
          <div className="flex items-center gap-6">
            <a href="#home" className="text-sm text-fg/50 transition hover:text-fg">
              Privacy Policy
            </a>
            <a href="#home" className="text-sm text-fg/50 transition hover:text-fg">
              Terms of Service
